...
const taskTemplates = [{component: {subsystem: 'brake'}, description: 'adjust brake', complexity: 'LOW', unitPrice: 150}, 
    {component: {subsystem: 'cab'}, description: 'touch up cabin seat', complexity: 'HIGH', unitPrice: 250 }]
...
test('show all services with task', async () => {
    ...
    render(<ServiceContext value={new ServiceTransactions(transactions, jest.fn())}>
    ...
    await user.click(screen.queryByPlaceholderText('Find a suitable task'))
    // choose one
    ...
    expect(onNewServiceCreated).toBeCalledWith({"id": 10001, "mileageKm": 230000, 
        "sparePartUsages": [
            {"id": 990001, "margin": 0, 
            "order": {"id": 1000, "itemCode": "1000", "partName": "Engine Oil 20w-50", 
                "quantity": 100, "status": "ACTIVE", "supplierId": 60001, "unit": "litres", "unitPrice": 9.7}, 
                "orderId": 1000, "quantity": 20, "usageDate": "2022-02-03"}, 
            {"id": 990004, "margin": 0, 
            "order": {"id": 2000, "itemCode": "2000", "partName": "Oil Filter", 
                "quantity": 5, "status": "ACTIVE", "supplierId": 60002, "unit": "pc", "unitPrice": 29.5}, 
                "orderId": 2000, "quantity": 1, "usageDate": "2022-02-03"}], 
            "startDate": "2022-02-02", 
        "tasks": [{"quotedPrice": 150, "recordedDate": "2022-02-02", "remarks": undefined, "rid": expect.anything(), 
            "selectedTask": [{"complexity": "LOW", "component": {"subsystem": "brake"}, "description": "adjust brake", "unitPrice": 150}], 
            "taskId": undefined}], 
