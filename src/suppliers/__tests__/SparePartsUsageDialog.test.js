...
test('remove vehicle, return no workshop services at all', async () => {
    const user = userEvent.setup()
    ...
    const orders = [
        {id: 1000, itemCode: '100001', partName: 'Engine Oil', supplierId: 2001, quantity: 200, unit: 'litre', unitPrice: 8.8, status: 'ACTIVE'},
        {id: 1001, itemCode: '100001', partName: 'Transmission Oil', supplierId: 2002, quantity: 150, unit: 'litre', unitPrice: 230, status: 'ACTIVE'}
    ]
    ...
    expect(screen.queryAllByText(/Service started at/)).toHaveLength(0)

    expect(setLoading).not.toBeCalled()
    expect(refreshSparePartUsages).not.toBeCalled()
    expect(refreshServices).not.toBeCalled()
})