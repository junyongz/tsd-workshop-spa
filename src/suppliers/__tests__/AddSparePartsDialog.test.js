...
test('create a 2 items order, but pending actual DO', async() => {
    ...
    expect(screen.queryByPlaceholderText('Key in DO. #').validity.valueMissing).toBeTruthy()
    await user.click(screen.getByLabelText('Pending DO/Invoice'))
    expect(screen.queryByPlaceholderText('Key in DO. #').validity.valueMissing).toBeFalsy()
...
test('edit existing order, all used up, after clone, DO number empty', async() => {
    ...
    expect(screen.getByPlaceholderText('Key in DO. #')).toHaveValue('')
    expect(screen.getByPlaceholderText('Choose a supplier')).toHaveValue('TSD')
    expect(screen.getByPlaceholderText('Key in Invoice Date')).toHaveValue('')
})
...
test('edit existing order, only non-usage allow to be submitted', async() => {
    ...
    await user.click(screen.getByLabelText('Save orders'))
    expect(onSaveNewOrders).toBeCalledWith([
        {"deliveryOrderNo": "DO-00003", "id": 20002, 
            "invoiceDate": todayDateStr, "itemCode": undefined, "notes": "hi", 
...
test('edit existing order, with pending DO items', async() => {
    ...
    render(<SupplierOrderContext value={new SupplierOrders(mockOrders, jest.fn())}>
        <AddSparePartsDialog isShow={true} setShowDialog={setShowDialog} 
            existingOrder={[
        {"deliveryOrderNo": "PENDING-DO-20001", "id": 20001, "invoiceDate": todayDateStr, 
            "itemCode": "07411", "notes": "hello world", "partName": "Brake Pads Yz", 
            "quantity": 8, "sparePartId": 900001, "supplierId": 2002, 
            "totalPrice": 2000, "unit": "pc", "unitPrice": 250}, 
        {"deliveryOrderNo": "PENDING-DO-20001", "id": 20002, "invoiceDate": todayDateStr, 
            "itemCode": undefined, "notes": "hi", "partName": "Engine Oil", 
            "quantity": 200, "sparePartId": undefined, "supplierId": 2002, 
            "totalPrice": 1939.9999999999998, "unit": "pc", "unitPrice": 9.7}]}
            onSaveNewOrders={onSaveNewOrders} sparePartUsages={[
    ...
    expect(screen.queryByText('Update Spare Parts')).toBeInTheDocument()
    expect(screen.getByPlaceholderText('Key in DO. #')).toBeEnabled()
    expect(screen.getAllByPlaceholderText('Key in item code')[0]).toBeDisabled()
    expect(screen.getAllByPlaceholderText('Key in item code')[1]).toBeEnabled()
    expect(screen.getByLabelText('Add new order')).toBeDisabled()
    ...
    await user.click(screen.getByLabelText('Save orders'))
    expect(onSaveNewOrders).toBeCalledWith([
        {"deliveryOrderNo": "DO-99110000", "id": 20001, 
            "invoiceDate": todayDateStr, "itemCode": "07411", "notes": "hello world", 
            "partName": "Brake Pads Yz", "quantity": 8, "sparePartId": 900001, 
            "supplierId": 2002, "totalPrice": 2000, "unit": "pc", "unitPrice": 250}, 
        {"deliveryOrderNo": "DO-99110000", "id": 20002, 
            "invoiceDate": todayDateStr, "itemCode": undefined, "notes": "hi", 
            "partName": "Engine Oil", "quantity": 200, "sparePartId": undefined, 
            "supplierId": 2002, "totalPrice": 1939.9999999999998, "unit": "pc", "unitPrice": 9.7}], 
            expect.anything())
