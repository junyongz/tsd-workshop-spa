...
export default function SparePartsSubDialog({
    items, setItems, spareParts, orders=new SupplierOrders(), suppliers, sparePartUsages, sparePartsMargin
}) {
            ...
            <ListGroup.Item key={i}>
                <Row>                                
                    <Col xs="12" xl="6" className="mb-3 mb-xl-0">
                        ...
                        <Typeahead
                            ...
                            renderMenuItemChildren={(option) => {
                                const order = orders.byId(option.orderId)
                    ...
                    <Col xs="6" xl="2" className="mb-3 mb-xl-0">
                        <InputGroup>
                            <Form.Control onChange={(e) => updatePriceByQuantity(e.target.value, i)} required step={decimalPointUomAvailable(v?.unit) ? 0.1 : 1} type="number" name="quantity" min="1" max={(v.selectedSpareParts[0] && remainingQuantity(orders.byId(v.selectedSpareParts[0].orderId), sparePartUsages)) || 0} placeholder="Quantity" value={v?.quantity}/>
